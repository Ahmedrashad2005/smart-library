...
export const coverMaxBytes = (): number =>
  Math.floor((Number(process.env.BOOK_COVER_MAX_MB) || 5) * 1024 * 1024);
export type UploadedCoverFile = {
  originalname: string;
  mimetype: string;
  size: number;
  buffer: Buffer;
...
export class BookCoverService {
  readonly root = resolve(
    process.env.BOOK_COVER_STORAGE_DIR ?? resolve(process.cwd(), 'uploads', 'books', 'covers'),
  );
  constructor(
    ...
    try {
      const updated = await this.prisma.$transaction(async (tx) => {
        const result = await tx.book.update({
          where: { id: bookId },
          data: {
            coverImageUrl: `${process.env.COVER_PUBLIC_BASE_URL ?? 'http://localhost:3000'}/api/v1/books/${bookId}/cover/${key}`,
          },
        });
        await tx.auditLog.create({
          data: {
            actorId: actor.id,
            action: book.coverImageUrl ? 'BOOK_COVER_REPLACED' : 'BOOK_COVER_UPLOADED',
            entityType: 'book',
            entityId: bookId,
            oldValues: { coverImageUrl: book.coverImageUrl },
            newValues: { coverImageUrl: result.coverImageUrl },
          },
        });
        return result;
      });
  ...
  async stream(bookId: string, key: string) {
    ...
    if (!book || !book.coverImageUrl?.endsWith(`/${key}`))
      throw new NotFoundException('Book cover not found');
    const safe = basename(key);
    const filePath = resolve(this.root, safe);
    try {
      ...
      return {
        ...
        size: details.size,
        mimeType:
          extname(safe).toLowerCase() === '.jpg'
            ? 'image/jpeg'
            : extname(safe).toLowerCase() === '.png'
              ? 'image/png'
              : 'image/webp',
      };
    } catch {
